...
static void pushutfchar(lua_State *L, int arg) {
	...
	((void)((0 <= code && code <= 0x10FFFF) || luaL_argerror(L, (arg), ("value out of range"))));
...
extern int luaopen_utf8(lua_State *L) {
	(luaL_checkversion_(L, 503, (sizeof(lua_Integer) * 16 + sizeof(lua_Number))), lua_createtable(L, 0, sizeof(funcs) / sizeof((funcs)[0]) - 1), luaL_setfuncs(L, funcs, 0));
	lua_pushlstring(L, "[\0-\x7F\xC2-\xF4][\x80-\xBF]*", sizeof("[\0-\x7F\xC2-\xF4][\x80-\xBF]*") / sizeof(char) - 1);
