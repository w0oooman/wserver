...
static int db_getinfo(lua_State *L) {
	...
	const char *options = (luaL_optlstring(L, (arg + 2), ("flnStu"), ((void *)0)));
	checkstack(L, L1, 3);
...
static int checkupval(lua_State *L, int argf, int argnup) {
	...
	((void)(((lua_getupvalue(L, argf, nup) != ((void *)0))) || luaL_argerror(L, (argnup), ("invalid upvalue index"))));
	return nup;
}
...
static int db_sethook(lua_State *L) {
	...
	if (lua_rawgetp(L, (-1000000 - 1000), &HOOKKEY) == 0) {
		...
		lua_pushvalue(L, -1);
		lua_rawsetp(L, (-1000000 - 1000), &HOOKKEY);
		lua_pushstring(L, "k");
...
static int db_debug(lua_State *L) {
	for (;
		...
		char buffer[250];
		(fprintf((&__iob_func()[2]), ("%s"), ("lua_debug> ")), fflush((&__iob_func()[2])));
		if (fgets(buffer, sizeof(buffer), (&__iob_func()[0])) == 0 ||
			strcmp(buffer, "cont\n") == 0)
			return 0;
...
extern int luaopen_debug(lua_State *L) {
	(luaL_checkversion_(L, 503, (sizeof(lua_Integer) * 16 + sizeof(lua_Number))), lua_createtable(L, 0, sizeof(dblib) / sizeof((dblib)[0]) - 1), luaL_setfuncs(L, dblib, 0));
	return 1;
}
