#ifndef __MYSQLDBPROCESS_H__
#define __MYSQLDBPROCESS_H__
#define __LCC__
#include <iostream>
#include <strstream>
#include "mysql.h"
...
namespace mysql
{
    ...
    public:
        ...
        public:
            ...
            TempBind operator [] ( int i ) 
			{   
				if (i >= nbind)
				{
					Log("【error:】bind[%d] overflow!,the sizeof(bind)/sizeof(bind[0]) = %d\n", i, nbind);
					return TempBind(bind[nbind-1], null[nbind-1], lens[nbind-1]);
				}
				return TempBind( bind[ i ], null[ i ], lens[ i ] ); 
			}
    ...
    public:
	    ...
	    //初始化数据库连接
	    bool InitDB( LPCTSTR UserName, LPCTSTR PassWord, LPCTSTR DBIP, LPCTSTR DBName, DWORD dwPort=3306 );
