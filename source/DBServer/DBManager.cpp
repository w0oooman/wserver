...
void CDBManager::DB2Redis(CUserData *pUserData)
{
	...
	redisCmd << "hmset" << ROLE_FORWARD << pUserData->m_dwRoleID <<
...
void CDBManager::Redis2Memory(CUserData *pUserData, DWORD dwUserID)
{
	...
	stringstream redisCmd;
	redisCmd << "hmget" << ROLE_FORWARD << dwUserID <<
...
void CDBManager::SaveUserPoint(DWORD dwID, DWORD dwPoint, bool bSaveDB/* = false*/)
{
	//保存redis
	stringstream redisCmd;
	redisCmd << "hset" << ROLE_FORWARD << dwID <<
		g_userdata[e_userpoint] << dwPoint;
	string cmd = redisCmd.str();
	redis_hset(cmd.c_str());

	//保存DB
	if (bSaveDB)
	{
		UpdateDBData(UPDATE_USER_POINT, dwPoint, dwID);
	}
}