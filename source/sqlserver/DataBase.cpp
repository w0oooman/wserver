...
bool __cdecl CDataBase::SetConnectionInfo(DWORD dwDBAddr, WORD wPort, LPCTSTR szDBName, LPCTSTR szUser, LPCTSTR szPassword)
{
	...
	try
	{
		//构造连接字符串 去MFC 特性,CString 改成 string
...
//获取参数 去除MFC特性 COleDateTime 替换成 tagTime,但没测试，不知道结果是不是正确...whb
...
LPCTSTR CDataBase::GetComErrorDescribe(CComError & ComError)
{
	...
	CHAR buf[1024] = { 0 };
	wsprintf(buf, TEXT("ADO 错误：0x%8x，%s"), ComError.Error(), (LPCTSTR)bstrDescribe);
	m_strConnect = buf;
