...
//构造函数
CDataBaseAide::CDataBaseAide(IDataBase * pIDataBase)
{
	m_pIDataBase=new CDataBase;
	SetDataBase(m_pIDataBase);
}
//析构函数
CDataBaseAide::~CDataBaseAide()
{
	...
	{
		try{delete m_pIDataBase;}
		catch(...){}
		m_pIDataBase=NULL;
	}
}

bool CDataBaseAide::SetDataBase(IDataBase *pIUnKnownEx)
{
...
IDataBase *CDataBaseAide::GetDataBase()
{
	if (m_pIDataBase != NULL)
		return m_pIDataBase;
	return NULL;
...
{
	DATE date = GetValue_TimeStamp(pszItem);
	if(date != 0)
	{
...
DATE CDataBaseAide::GetValue_TimeStamp(LPCTSTR pszItem)
{
	tagTime date;
	if (m_pIDataBase != NULL)
	{
		((CDataBase*)m_pIDataBase)->GetFieldValue(pszItem, date);
		return date.time;
	}
	return 0;
}
