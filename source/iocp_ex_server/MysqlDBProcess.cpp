...
const  MYSQL * CQuery::GetMySql( void )
{
	return m_pData;
}

bool CQuery::Connect( LPCSTR host, LPCSTR user, LPCSTR passwd, LPCSTR db, 
...
bool CQuery::InitDB(LPCTSTR UserName, LPCTSTR PassWord, LPCTSTR DBIP, LPCTSTR DBName, DWORD dwPort)
{
	// 初始化mysql
	...
	//mysql_options(m_pData, MYSQL_REPORT_DATA_TRUNCATION, "test");
	...
	//mysql_stmt_attr_set(m_pstmt, STMT_ATTR_CURSOR_TYPE, (void*)&type);
...
void CQuery::LogError( LPCTSTR FuncName, LPCTSTR Error)
{	
...
/*
int CQuery::ExecuteSQL( LPCSTR sql, MYSQL_RES **res )
{
	m_AffectedRows = 0;
	MYSQL_RES *temp = NULL;
...
int  CQuery::ExecuteSqlCmd( LPCSTR sql )
{
	assert( sql != NULL );
...
BOOL CQuery::KeepAlive( DWORD timeMargin, LPCSTR who )
{
	if ( (int)timeGetTime() - (int)prevCheckTime > (int)timeMargin )
	{
        ...
        if ( int err = mysql_ping( m_pData ) )
		{
			...
			Clear();
			BOOL succ = InitDB( m_User.c_str(), m_Password.c_str(), m_Host.c_str(), m_DBName.c_str() );
            char temp[256];
            sprintf( temp, succ ? "Ado[%d] [%s]连接断开, 重连接成功!" : "Ado[%d] [%s]连接断开, 重连接失败！！！", err, who ? who : "" );
            LogError( "CQuery::KeepAlive", temp ); 
            return succ;
		}
        //else
        //{
...
int CQuery::ExecuteStmt( const void *sql, size_t size, QueryResult *qr, Arguments args )
{
	...
	//不加下面代码可能会出现“ Commands out of sync; you can't run this command now” by whb
	//while (mysql_stmt_next_result(m_pstmt))
	//	mysql_stmt_store_result(m_pstmt);
	do
	{
		MYSQL_RES* res = mysql_store_result(m_pData);
		if (res) mysql_free_result(res);
	} while ((0 == mysql_next_result(m_pData)));
...
int CQuery::ExecuteSql( const void *sql, size_t size, QueryResult *qr )
{
	do
	{
		MYSQL_RES* res = mysql_store_result(m_pData);
...
int CQuery::Init( LPCSTR ConfigFileName)
{
	IniFiles  IniFile;
	...
	return InitDB(m_User.c_str(), m_Password.c_str(), m_Host.c_str(), m_DBName.c_str(), m_Port );
}

void CQuery::Backup( LPCSTR path, LPCSTR title )
{
...
// #########################BIND Begin##############################
CQuery::TempBind::TempBind( MYSQL_BIND &a, my_bool &b, unsigned long &c ) : 
...
void CQuery::TempBind::BindBlob( void *data, size_t size ) 
{
...
void CQuery::TempBind::BindString( const char *data, size_t size )
{
    lens = ( unsigned long )size;
...
void CQuery::TempBind::BindUTiny( unsigned char *data )
{
...
void CQuery::TempBind::BindLong( long *data ) 
{
...
void CQuery::TempBind::BindULong( unsigned long *data ) 
{
    bind.buffer_type = MYSQL_TYPE_LONG;
    bind.buffer = ( char* )data;
    bind.is_null = &null;
...
void CQuery::TempBind::BindLongLong( __int64 *data ) 
{
...
void CQuery::TempBind::BindULongLong( unsigned __int64 *data ) 
{
    bind.buffer_type = MYSQL_TYPE_LONGLONG;
    bind.buffer = ( char* )data;
    bind.is_null = &null;
...
// #########################QueryResult Begin####################
void CQuery::QueryResult::Release() 
{
	if ( res )
...
int CQuery::QueryResult::AffectedRows() 
{ 
...
int CQuery::QueryResult::FetchResult( MYSQL_BIND *bind )
{
    if ( stmt == NULL || *stmt == NULL )
        return -1;
...
int CQuery::QueryResult::FetchResult()
{
    if ( res == NULL )
        return -1;
...
int CQuery::ProcedureHelper::Prepare( LPCSTR procName )
{
    strProc = procName;
	char sql[1024];
...
int CQuery::ProcedureHelper::Execute()
{
    int len = 0;
    int result = 0;
    DWORD idxPrepare = 0;
...
lite::Variant CQuery::ProcedureHelper::GetField( int idx )
{
...
lite::Variant & CQuery::ProcedureHelper::GetParam( int idx )
{
...
int CQuery::ProcedureHelper::BindParam( int idx, lite::Variant &param )
{
...
CQuery::ProcedureHelper::ProcedureHelper( CQuery &qry ) : 
    query( qry ) 
{
...
CQuery::ProcedureHelper::~ProcedureHelper()
{
}
